(() => {
  ...
  // Feedを追加する場合は、ここに { name, url, className, keywords } を追加します。
  const RSS_SOURCES = [
    {
      name: '5chまとめ',
      url: 'https://5chant.com/feed',
      className: 'source-5ch',
      keywords: ['MTG', 'マジック：ザ・ギャザリング', 'マジック・ザ・ギャザリング', 'ギャザリング', 'カードゲーム', 'ウィザーズ']
    },
    {
      name: 'MTG公式',
      url: 'https://mtg-jp.com/index.rdf',
      className: 'source-official'
    },
    {
      name: 'MTGGoldfish',
      url: 'https://www.mtggoldfish.com/feed',
      className: 'source-goldfish'
    }
  ];
  ...
  async function fetchFeed(source) {
    ...
    try {
      ...
      if (data.status !== 'ok' || !Array.isArray(data.items)) {
        throw new Error(data.message || 'RSSデータが取得できませんでした');
      }
  ...
  async function fetchMixedRSS() {
    const container = document.getElementById('rss-timeline-list');
    const status = document.getElementById('rss-timeline-status');
    if (!container) return;

    status?.replaceChildren(document.createTextNode('最新情報を取得中…'));
    ...
    const visibleArticles = uniqueArticles.slice(0, MAX_ITEMS);
    container.replaceChildren();

    if (!visibleArticles.length) {
      const empty = document.createElement('p');
      empty.className = 'rss-timeline-empty';
      empty.textContent = '現在、表示できる最新記事がありません。';
      container.appendChild(empty);
    } else {
      visibleArticles.forEach((article, index) => {
        container.appendChild(createItem(article, index));
      });
    }

    if (status) {
      status.textContent = successCount > 0
        ? `${successCount}/${RSS_SOURCES.length}サイトから取得`
        : 'RSSを取得できませんでした';
    }
  }
